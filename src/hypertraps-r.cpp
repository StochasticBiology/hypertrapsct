...
double LikelihoodMultiple(int *targ, double *P, int LEN, int *startpos, double tau1, double tau2, int model)
{
  ...
  for(i = 0; i < LEN; i++)
  {
    ...
    if(targ[i] == 2 && !(tau1 == 0 && tau2 == INFINITY)) {
      Rprintf("Uncertain observations not currently supported for the continuous time picture! Please re-run with the discrete time picture.\n");
      myexit(0);
    }
    if(targ[i] == 0 && startpos[i] == 1) {
      Rprintf("Wrong ordering, or some other problem with input file. Data file rows should be ordered ancestor then descendant! Problem transition was:\n");
      for(j = 0; j < LEN; j++) Rprintf("%i", startpos[j]);
      Rprintf(" -> ");
      for(j = 0; j < LEN; j++) Rprintf("%i", targ[j]);
      myexit(0);
    }
  ...
  for(r = n0; r < n1; r++)
  {
    ...
    if(SUPERVERBOSE)
    {
      Rprintf("At step %i averaged summand is %e\n", i, summand[r]);
    }
    ...
    for(n = 0; n < BANK; n++)
    {
      ...
      for(i = 0; i < n1-n0; i++)
      {
        ...
        for(j = 0; j < n1-n0; j++)
        {
          ...
          if(SPECTRUM_VERBOSE)
            Rprintf("step %i looking at step %i recbeta %.4f vi %.4f\n", i, j, recbeta[LEN*n+j], vi);
        }
        ...
        if(SPECTRUM_VERBOSE)
          Rprintf("walker %i: stepx %i vi %.4f betaci %.4f u %.4f | sumI1 %.4f sumI2 %.4f\n (tau1 %f tau2 %f)\n", n, i, vi, betaci, u, sumI1, sumI2, tau1, tau2);
      }
      
      if(sumI1 < 0 || sumI2 < 0)
      {
        if(num_error == 0) {
          Rprintf("I got a negative value for I1 (%e) or I2 (%e), which shouldn't happen and suggests a lack of numerical convergence. This can happen with large numbers of features. I'm setting to zero but this may be worth examining.\n", sumI1, sumI2);
        }
        if(sumI1 < 0) {
          if(-sumI1 > num_error) {
            num_error = -sumI1;
            Rprintf("New scale of numerical error %e\n", num_error);
          }
          sumI1 = 0;
        }
        if(sumI2 < 0) {
...
double GetLikelihoodCoalescentChange(int *matrix, int len, int ntarg, double *ntrans, double *tau1s, double *tau2s, int model, int PLI)
{
  ...
  for(i = 0; i < ntarg/2; i++)
  {
    ...
    {
      Rprintf("Target %i: ", i);
      for(j = 0; j < len; j++) Rprintf("%i", matrix[2*i*len+len+j]);
      Rprintf(" parent is: " );
      for(j = 0; j < len; j++) {  startpos[j] = matrix[2*i*len+j]; Rprintf("%i", startpos[j]); }
      Rprintf("\n");
    }
    ...
    if(tlik <= 0)
    {
      Rprintf("Somehow I have a negative or zero likelihood, suggesting a lack of numerical convergence. Terminating to avoid unreliable posteriors.\n");
      Rprintf("This was at observation %i, which is\n", i);
      for(j = 0; j < len; j++) Rprintf("%i", matrix[2*i*len+len+j]);
      Rprintf(" parent is: " );
      for(j = 0; j < len; j++) {  startpos[j] = matrix[2*i*len+j]; Rprintf("%i", startpos[j]); }
      Rprintf("\n");
...
void Label(char *names, int len, char *fname)
{
  ...
  if(fp == NULL)
  {
    Rprintf("Didn't find feature label file %s, using default labels\n", fname);
    for(i = 0; i < len; i++)
    {
      snprintf(&names[i*FLEN], FLEN, "f%i", i);
    }
  }
  else
  {
                       ...
                       int outputtransitions = 0)
{
  ...
  if(featurenames.isUsable()) {
    ...
    for(i = 0; i < len; i++)
    {
      snprintf(&names[i*FLEN], FLEN, "%s", (char*)_featurenames[i]);
    }
  ...
  if(filelabel == 0)
    snprintf(labelstr, sizeof(labelstr), "%s", postfile);
